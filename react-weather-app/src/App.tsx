...
const App = () => {
    ...
    // Функція для визначення мови на основі назви міста
    const detectLanguage = (cityName: string): string => {
      // Українські символи
      if (/[а-яґєіїА-ЯҐЄІЇ]/u.test(cityName)) {
        return 'uk';
      }
    ...
    const fetchForecastData = async (city: string) => {
      ...
      try {
        const lang = detectLanguage(city);
        const response = await fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${city}&appid=${apiKey}&units=metric&lang=${lang}`);
        if (!response.ok) {
    ...
    // format date з урахуванням мови міста
